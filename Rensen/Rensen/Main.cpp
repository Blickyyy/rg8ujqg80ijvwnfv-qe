#include "Head.h"
#include "CS2_SDK.h"
const float Rensen_Version = 0.1;//程序版本
const string Rensen_ReleaseDate = "GER[2024-09-12 22:00]";//程序发布日期时间
namespace Control_Var//套用到菜单的调试变量 (例如功能开关)
{
	EasyGUI::EasyGUI GUI_VAR; EasyGUI::EasyGUI_IO GUI_IO; BOOL Menu_Open = true; string Preset_Folder = "devire";//菜单初始化变量
	//----------------------------------------------------------------------------------------------
	...
	float UI_Setting_MenuAnimation = 2;
...
void Thread_Menu() noexcept//菜单线程 (提供给使用者丰富的自定义选项)
{
	...
	GUI_VAR.Window_Create(1010, 940, "devire - Menu", true);//创建置顶GUI绘制窗口
	...
	while (true)//默认英文版菜单
	{
		...
		if (!GUI_VAR.Window_Move() && Menu_Open)//不在移动窗口时绘制GUI
		{
			...
			GUI_VAR.GUI_Block(20, 20, 40, "", 110, false); GUI_VAR.In_DrawString(36, 35, "devire", GUI_IO.GUIColor.Min_Bri(200), "Verdana", 25);//Rensen标志
			GUI_VAR.GUI_Block_Panel<class CLASS_Block_Panel>(20, 70, 110, GUI_VAR.Window_GetSize().y - 90, "", { "Legit","Visual","Misc","Infolist","Settings","Scripting" }, UI_Panel, 25);//大体区块选择
			if (UI_Panel == 0)//Legit
			{
				...
				GUI_VAR.GUI_Slider<float, class CLASS_Block_MagnetAim_2>(Block_MagnetAim, 4, "Smooth", 0, 7, UI_Legit_MagnetAim_Smooth);
			...
			else if (UI_Panel == 3)//List
			{
				...
				GUI_VAR.GUI_Checkbox(Block_DebugControl, 4, "Invert Color", Debug_Control_Var::Checkbox_2);
			...
			else if (UI_Panel == 4)//Setting
			{
				...
				GUI_VAR.GUI_Text(Block_About, 1, "devire", GUI_IO.GUIColor);
				...
				GUI_VAR.GUI_Text(Block_About, 4, "Author: devil", { 100,100,100 });
				static BOOL OpenGithubURL; GUI_VAR.GUI_Button_Small({ Block_About.x + 10,Block_About.y }, 4, OpenGithubURL);
				if (OpenGithubURL)System::Open_Website("https://discord.gg/Rbphpud8dQ");//打开作者Github主题页面
				...
				//if (GithubRepositories)System::Open_Website("coming soon");//打开Github项目地址
				static BOOL RestartMenu; GUI_VAR.GUI_Button(Block_Menu, 8, "Restart menu", RestartMenu, 75);
				if (RestartMenu) { GUI_VAR.Window_SetTitle("devire - Restarting"); System::Self_Restart(); }//重启菜单
...
void Thread_Misc() noexcept//杂项线程 (一些菜单事件处理和杂项功能)
{
	...
	Window::Windows Window_Watermark; Window_Watermark.Create_RenderBlock_Alpha(Window::Get_Resolution().x, 50, "devire - Watermark");//创建水印透明窗口
	...
	Window::Windows Window_NightMode; Window_NightMode.Create_RenderBlock(Window::Get_Resolution().x, Window::Get_Resolution().y, "devire - NightMode");//夜晚模式窗口
	...
	while (true)
	{
		...
		if (UI_Misc_Watermark)//水印
		{
			...
			if (System::Sleep_Tick<class CLASS_WaterMark_WindowReload_Delay_>(200))//降低CPU占用
			{
				...
				if (!CS2_HWND)WaterMark_String = "devire | CS not found | " + System::Get_UserName() + " | " + System::Time_String();
				else { WaterMark_String = "devire | " + Advanced::LocalPlayer_Name() + " | " + System::Time_String(); WaterMark_String_Size = strlen(WaterMark_String.c_str()) * 5.2; }
...
void Thread_Funtion_BunnyHop() noexcept//功能线程: 连跳
{
	...
	while (true)
	{
		if (CS2_HWND && Global_IsShowWindow && Global_LocalPlayer.Health() && UI_Misc_BunnyHop && System::Get_Key(VK_SPACE))
		{
			...
			Sleep(System::Rand_Number(1, 8, System::Tick()));
		}
		else Sleep(20);
	}
}
	...
	while (true)
	{
		if (CS2_HWND && Global_IsShowWindow && Global_LocalPlayer.Health())//当CS窗口在最前端 && 本地人物活着
		{
			...
			if (UI_Legit_MagnetAim && !System::Get_Key(VK_LBUTTON) && Global_LocalPlayer.ActiveWeapon() != 0 && Global_LocalPlayer.ActiveWeapon(true) != 3 && Global_LocalPlayer.MoveSpeed() <= 150)//磁吸瞄准
			{
				...
				if (EligiblePlayers.MinFov <= Aim_Range && EligiblePlayers.MinFov >= 0.4)System::Mouse_Move(-EligiblePlayers.AimAngle.y * (7.f - UI_Legit_MagnetAim_Smooth), EligiblePlayers.AimAngle.x * (7.f - UI_Legit_MagnetAim_Smooth) * 0.7);
			}
		}
		else Sleep(20);
	}
...
void Thread_Funtion_PlayerESP() noexcept//功能线程: 透视和一些视觉杂项
{
	...
	Window::Windows RenderWindow; RenderWindow.Create_RenderBlock_Alpha(Window::Get_Resolution().x, Window::Get_Resolution().y, "devire - PlayerESP");//创建绘制覆盖窗口
...
void Thread_Funtion_EntityESP() noexcept//功能线程: 实体透视
{
	...
	Window::Windows RenderWindow; RenderWindow.Create_RenderBlock(Window::Get_Resolution().x, Window::Get_Resolution().y, "devire - EntityESP");
...
void Thread_Funtion_Radar() noexcept//功能线程: 雷达
{
	...
	Window::Windows Radar_Window; Radar_Window.Create_Window(UI_Visual_Radar_Size, UI_Visual_Radar_Size + 15, "devire - Radar", true);//创建雷达绘制窗口
	...
	while (true)
	{
		...
		if ((Global_IsShowWindow || Menu_Open || Window::Get_WindowEnable(Radar_Window.Get_HWND())) && UI_Visual_Radar)//当CS窗口在最前端
		{
			...
			if (!Radar_Window.Window_Move(15))//移动雷达窗口
			{
				...
				Radar_Paint.Render_String(3 + 1, 1 + 1, "devire - Radar", "Small Fonts", 12, { 0,0,1 }, false);//标题阴影
				Radar_Paint.Render_String(3, 1, "devire - Radar", "Small Fonts", 12, GUI_IO.GUIColor, false);//标题
...
int main() noexcept//主线程 (加载多线程, 一些杂项功能)
{
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	if (FindWindow(0, L"devire - Menu")) { Window::Message_Box("devire Error", "The program is already running.", MB_ICONSTOP); exit(0); }//防止多开程序
	//----------------------------------------------------------------------------------------------------------------------------------
	...
	//if (!Attest) { Window::Message_Box("devire Attest - " + System::Get_UserName(), "Your identity cannot be passed.\n\nUnable to access from Chinese IP.\n\nAuthor: https://github.com/Coslly\n", MB_ICONSTOP); exit(0); }//未被认证则直接退出
	//----------------------------------------------------------------------------------------------------------------------------------
	...
	//	if (Variable::string_float_(Version) > Rensen_Version && Window::Message_Box("devire Update", "A new version has been released.\nDo you want to update now?\nIt may take tens of seconds.\n\nYou will be updated to Ver[" + Version + "]", MB_YESNO | MB_ICONASTERISK) == 6)
	//	{
	...
	printf("Welcome to devire for Counter-Strike 2.\n");//作者留言
